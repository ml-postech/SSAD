...
class XUMXControl(BaseModel):
    ...
    def forward(self, wav, controls):
        ...
        # Transform
        # wav [B, channel, time]
        # controls [B, sources, channel, time]
        mixture, ang = self.encoder(wav)
        # [Tb, batch, channel, Fb]

        # TODO: overlap in STFT
        Tb = mixture.shape[0]
        B, src, ch, time = controls.shape
        controls_tmp = controls.reshape(B, src * ch, time)
        control_spec = F.adaptive_max_pool1d(controls_tmp, output_size=Tb)
        control_spec = control_spec.reshape(B, src, ch, Tb, 1)
        # control_spec [B, sources, channel, Tb, 1]

        # Estimate masks
        est_masks = self.forward_masker(mixture.clone(), control_spec)
    ...
    def forward_masker(self, input_spec, controls_spec):
        shapes = input_spec.data.shape

        # inputs : [Tb, B, ch, Fb]
        # controls : [B, src, ch, Tb, 1]
        controls_spec = controls_spec.permute(1,3,0,2,4)
        # controls : [src, Tb, B, ch, 1]
